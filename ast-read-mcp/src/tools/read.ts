...
const ReadFileSchema = z.object({
  ...
  mode: z
    ...
    .default("full")
    .describe(
      "Reading mode: 'full' = entire file, 'outline' = structure only, 'target' = specific entity, 'lines' = specific line range"
    ),
  target: z
    .string()
    .optional()
    .describe(
      "Target to extract (requires mode='target'). Format: 'function:name', 'class:name', 'class:ClassName.methodName', 'method:methodName', 'imports', 'exports'"
    ),
  line: z
    .number()
    .optional()
...
interface FunctionInfo {
  ...
  async: boolean;
  signature: string;
  type: "function_declaration" | "arrow_function" | "function_expression" | "method";
  jsdoc?: string;
}
...
export class AstReadTool {
  static getSchema() {
    return {
      name: "ast_read_file",
      description:
        "Read files with AST-awareness and structural understanding. FIXED: Now uses Babel parser for 95%+ reliability.",
      inputSchema: {
        type: "object",
        properties: {
          file_path: {
          ...
          mode: {
            ...
            description:
              "Reading mode: 'full' = entire file, 'outline' = structure only, 'target' = specific entity",
            default: "full",
          },
          target: {
            type: "string",
            description:
              "Target to extract (requires mode='target'). Format: 'function:name', 'class:name', 'imports', 'exports'",
          },
          context: {
            type: "boolean",
    ...
    try {
      ...
      switch (params.mode) {
        ...
        case "lines":
          if (!params.line) {
            return this.createError(
              "line parameter required when mode='lines'",
              "MISSING_LINE",
              params.file_path,
              params.mode
            );
        ...
        case "target":
          if (!params.target) {
            return this.createError(
              "target parameter required when mode='target'",
              "MISSING_TARGET",
              params.file_path,
              params.mode
            );
   ...
   */
  private getHelpfulHint(error: Error): string {
   ...
   * Read full file
   */
  private readFull(filePath: string, content: string) {
   ...
   */
  private readLines(params: ReadFileInput, content: string) {
    const lines = content.split("\n");
    ...
    if (targetLine < 1 || targetLine > lines.length) {
      return this.createError(
        `Line ${targetLine} out of range (file has ${lines.length} lines)`,
        "LINE_OUT_OF_RANGE",
        params.file_path,
        params.mode
      );
    ...
    try {
      ...
      traverseFn(ast, {
        ...
        ImportDeclaration(path: any) {
          const node = path.node;
          const imported = node.specifiers.map((spec: any) => {
  ...
  private readTarget(params: ReadFileInput, content: string, language: string) {
    ...
    let targetName: string = targetPath;
    ...
    try {
      ...
      const extractCode = (start: number, end: number, targetType: string, targetName: string, className?: string) => {
      ...
      traverseFn(ast, {
        ClassDeclaration(path: any) {
          ...
          if ((targetType === "class" || targetType === "method") && className && currentClassName === className) {
            const method = node.body.body.find((member: any) => 
              t.isClassMethod(member) && t.isIdentifier(member.key) && member.key.name === targetName
            );
      ...
      return this.createError(
        `Failed to extract target: ${error.message}`,
        error.constructor.name,
        params.file_path,
        params.mode
      );
