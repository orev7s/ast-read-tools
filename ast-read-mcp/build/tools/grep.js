...
import { parseWithBabel, extractJSDoc, readFileContent, getLinesWithContext, findCodeFiles, findSearchableFiles, isCodeFile, } from "./shared.js";
/**
...
Example: Find all functions named "handle*":
  { pattern: "handle.*", type: "function", path: "./src" }`,
    ...
    async execute(input) {
        try {
            ...
            if (!fs.existsSync(searchPath)) {
                return this.createError(`Path not found: ${searchPath}`, "PATH_NOT_FOUND", searchPath);
            }
            ...
            if (error instanceof z.ZodError) {
                return this.createError(`Invalid input: ${error.issues.map((e) => e.message).join(", ")}`, "VALIDATION_ERROR");
            }
            return this.createError(error instanceof Error ? error.message : "Unknown error", "UNKNOWN_ERROR");
        }
    }
    /**
     * Search a single file for matches
     */
    searchFile(filePath, params) {
        try {
            ...
            const matchesModifiers = (node, parent) => {
                ...
                if (node.static)
                    nodeModifiers.push('static');
                // Check for export
                if (parent && t.isExportNamedDeclaration(parent))
                    nodeModifiers.push('export');
            ...
            traverseFn(ast, {
                ...
                FunctionDeclaration: (path) => {
                    ...
                    if (!matchesModifiers(node, parent))
                        return;
                    ...
                    if (isModifierPattern || nameMatches) {
                        if (node.id || isModifierPattern) {
                            ...
                            const contextData = getLinesWithContext(lines, line, contextBefore, contextAfter);
                            matches.push({
                                file: filePath,
                                line,
                ...
                VariableDeclarator: (path) => {
                    if (searchType !== "all" && searchType !== "function" && searchType !== "variable")
                        return;
                    const node = path.node;
                    ...
                    if (t.isIdentifier(node.id)) {
                        const isFunctionValue = t.isArrowFunctionExpression(node.init) ||
                            t.isFunctionExpression(node.init);
                        // Check modifiers for functions and variables
                        if (!matchesModifiers(node.init || node, grandParent || parent))
                            return;
                        // Match functions or variables
                        if ((searchType === "function" && isFunctionValue) ||
                ...
                ClassMethod: (path) => {
                    ...
                    const node = path.node;
                    // Check modifiers
                    if (!matchesModifiers(node, path.parent))
                        return;
                    if (t.isIdentifier(node.key)) {
                        ...
                        if (isModifierPattern || nameMatches) {
                            ...
                            if (node.async)
                                modifiers.push('async');
                            if (node.accessibility)
                                modifiers.push(node.accessibility); // private, public, protected
                            if (node.kind === 'get')
                                modifiers.push('get');
                            if (node.kind === 'set')
                                modifiers.push('set');
                ...
                ExportNamedDeclaration: (path) => {
                    if (searchType !== "all" && searchType !== "export")
                        return;
                    const node = path.node;
                    const line = node.loc?.start.line || 0;
                                    ...
                                    for (const prop of node.right.properties) {
                                        if (t.isObjectProperty(prop) && t.isIdentifier(prop.key)) {
                                            if (regex.test(prop.key.name)) {
                                                ...
                                                const propContextData = getLinesWithContext(lines, propLine, contextBefore, contextAfter);
                                                matches.push({
                                                    file: filePath,
                                                    line: propLine,
                ...
                Identifier: (path) => {
                    ...
                    if (regex.test(node.name)) {
                        ...
                        const isDuplicate = matches.some(m => m.line === line && m.column === (node.loc?.start.column || 0));
                        if (!isDuplicate) {
                            matches.push({
                                file: filePath,
                                line,
     ...
     */
    searchFileAsText(filePath, content, lines, params) {
        const matches = [];
        // Create regex pattern
        ...
        lines.forEach((line, index) => {
            ...
            while ((match = regex.exec(line)) !== null) {
                const contextData = getLinesWithContext(lines, lineNumber, contextBefore, contextAfter);
                matches.push({
                    file: filePath,
                    line: lineNumber,
                    column: match.index,
     ...
     */
    createError(message, type, filePath) {
        return {
            success: false,
            error: {
                message,
